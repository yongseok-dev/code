...
입력
첫째 줄에는 참여하는 사람 수 N이 주어지고 그 다음 줄부터
...
for i in range(n):
    ...
    if d.count(max(d))==3:
        data.append(10000+max(d)*1000)
    elif d.count(max(d))==2:
        data.append(1000+max(d)*100)
    elif d.count(min(d))==2:
        data.append(1000+min(d)*100)
    else:
        data.append(max(d)*100)
print(max(data))
